#!/usr/bin/env python3
"""
Generate sitemap.xml from the project's HTML files.

Behavior:
- Walks all *.html under the project root
- Skips dev-helper files, 404.html, thank-you (form-confirmation), and any
  page that has <meta name="robots" content="noindex...">  (so we never
  send Google conflicting signals)
- For each indexed page, derives canonical URL from the file path
- <lastmod> comes from the file's last git-commit date (falls back to mtime
  if the file is untracked or git isn't available)
- Extracts the page's primary image from <meta property="og:image"> and
  adds an <image:image> entry with caption (og:image:alt) and title (<title>)

Run automatically on every Netlify deploy via netlify.toml [build].command.
Can also be run locally:  python3 build-sitemap.py
"""
...
SKIP_FILES = {
    "404.html",                      # error page
    "deploy-netlify-steps.html",     # local dev helper (also in .gitignore)
    "setup-github-steps.html",       # local dev helper (also in .gitignore)
}
# Directories to skip during walk
SKIP_DIRS = {".git", ".netlify", ".claude", "node_modules", "images", "css"}

# Priority by URL path (modern Google ignores priority but it's still
# meaningful for some search engines and harmless).
PRIORITY_BY_PATH = {
    "/": 1.0,
...
def get_lastmod(path: Path) -> str:
    """Return ISO-8601 last-modified for a file: git commit time if available."""
    try:
...
def build_sitemap() -> tuple[str, int, int]:
    ...
    lines = [
        ...
        '',
        '  <!-- Auto-generated by build-sitemap.py. Do not edit by hand. -->',
        '',
    ]
    image_count = 0
    for e in entries:
...
def main() -> int:
    ...
    print(f"sitemap.xml generated: {pages} URLs, {images} with images")
    return 0
